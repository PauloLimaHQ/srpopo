...
const { id, now, appendLog } = require('./store');
 ...
 */
function create(taskId, toolName, input) {
  const reqId = id();
  const entry = { id: reqId, taskId, toolName: String(toolName || 'tool'), input, createdAt: now(), settled: false, resolve: null, timer: null };
...
function settle(taskId, reqId, decision, reason) {
...
function decide(taskId, reqId, { behavior, message, updatedInput } = {}) {
  const decision = behavior === 'allow'
    ? { behavior: 'allow', ...(updatedInput && typeof updatedInput === 'object' ? { updatedInput } : {}) }
...
function rejectForTask(taskId, message = 'Run ended') {
  const m = byTask.get(taskId);
  if (!m) return;
