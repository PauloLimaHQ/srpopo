 ...
 * that watches tasks sitting in `validation` and, when their PR has a merge conflict
 ...
 * plain validation-column tasks (no live board tab required, and independent of
 * Autonomous Mode), while a bus subscription clears the "Resolving Conflicts"
 * label once the resumed run lands back in validation/failed/ready — the normal
 ...
 * in validation for a human when the engine could fix it itself.
 */
...
// or a user stop) — the runner already moved it to validation/failed/ready, so all
...
function onBus(msg: unknown): void {
  ...
  if (task.status === 'running' || task.status === 'code_review') return; // still in flight
...
// Periodic sweep for plain (non-autonomous) tasks: every validation-column task with
...
async function sweep(): Promise<void> {
  ...
  const candidates = db.tasks.filter(
    (t) => !t.archived && t.status === 'validation' && !t.resolvingConflicts && t.branch && !runner.isRunning(t.id),
  );
