/* Sr. Popo — icon set. Inline SVG, no build step, no dependencies.

   These are Lucide icons (https://lucide.dev, ISC-licensed) — the same open
   source set shadcn/ui ships with. We inline them so the UI stays dependency-
   light and works offline: no icon font, no network fetch, no bundler.

   Every glyph is a 24×24 stroke icon that inherits `currentColor`, so it picks
   up the surrounding text color and theming for free. Prefer these over emojis
   anywhere in the UI.

   Usage:
     • Static HTML — add a placeholder that hydrate() fills once on load:
         <span class="i" data-icon="settings"></span>
     • Dynamic markup (app.js) — call the renderer inside a template string:
         `<button>${srpopoIcons.svg('play')} Run</button>`
*/
(() => {
  'use strict';

  // name -> inner SVG markup (paths/shapes only). Add new glyphs here.
  const ICONS = {
    ...
    // crown — the orchestrator agent that plans a goal into worker tasks
    ...
    // theater masks — used for "persona" (an expert hat Claude wears)
    persona: '<path d="M18 11c-1.5 0-2.5.5-3 2"/><path d="M4 6a2 2 0 0 0-2 2v4a5 5 0 0 0 5 5 8 8 0 0 1 5 2 8 8 0 0 1 5-2 5 5 0 0 0 5-5V8a2 2 0 0 0-2-2h-3a8 8 0 0 0-5 2 8 8 0 0 0-5-2z"/><path d="M6 11c1.5 0 2.5.5 3 2"/>',
    // shield — used for interactive tool-permission prompts
    ...
    // file / image — attachment thumbnails in the task composer
    file: '<path d="M15 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V7Z"/><path d="M14 2v4a2 2 0 0 0 2 2h4"/>',
    image: '<rect width="18" height="18" x="3" y="3" rx="2"/><circle cx="9" cy="9" r="2"/><path d="m21 15-3.086-3.086a2 2 0 0 0-2.828 0L6 21"/>',
    // inbox — "create in backlog" (queue it without running it)
    inbox: '<polyline points="22 12 16 12 14 15 10 15 8 12 2 12"/><path d="M5.45 5.11 2 12v6a2 2 0 0 0 2 2h16a2 2 0 0 0 2-2v-6l-3.45-6.89A2 2 0 0 0 16.76 4H7.24a2 2 0 0 0-1.79 1.11z"/>',
    // type — the task title field
    ...
    // ellipsis — the standard "more actions" affordance on an overflow menu button
    ellipsis: '<circle cx="12" cy="12" r="1"/><circle cx="19" cy="12" r="1"/><circle cx="5" cy="12" r="1"/>',
    // message-circle-question — the "Ask Sr. Popo" entry point
    ...
    // grip-vertical — drag handle affordance for reorderable lists (repos)
    'grip-vertical': '<circle cx="9" cy="12" r="1"/><circle cx="9" cy="5" r="1"/><circle cx="9" cy="19" r="1"/><circle cx="15" cy="12" r="1"/><circle cx="15" cy="5" r="1"/><circle cx="15" cy="19" r="1"/>',
  };

  // Render an icon as an inline <svg> string. `opts.spin` adds the spin class,
  // `opts.cls` appends extra classes. Unknown names render nothing (safe no-op).
  function svg(name, opts = {}) {
    const body = ICONS[name];
    if (!body) return '';
    const cls = `icon${opts.spin ? ' icon-spin' : ''}${opts.cls ? ` ${opts.cls}` : ''}`;
    return `<svg class="${cls}" viewBox="0 0 24 24" fill="none" stroke="currentColor" ` +
      'stroke-width="2" stroke-linecap="round" stroke-linejoin="round" ' +
      `aria-hidden="true" focusable="false">${body}</svg>`;
  }

  // Fill every `[data-icon]` placeholder in `root` with its icon. Called once on
  // load to hydrate the static markup in index.html.
  function hydrate(root = document) {
    root.querySelectorAll('[data-icon]').forEach((el) => {
      el.innerHTML = svg(el.dataset.icon, { spin: el.hasAttribute('data-spin') });
    });
  }

  window.srpopoIcons = { svg, hydrate };
  hydrate();
})();
